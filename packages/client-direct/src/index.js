import { composeContext, elizaLogger, generateCaption, generateImage, generateMessageResponse, generateObject, getEmbeddingZeroVector, messageCompletionFooter, ModelClass, settings, stringToUuid, } from "@elizaos/core";
        ...
        this.app.post("/agents/:agentIdOrName/hyperfi/v1", async (req, res) => {
            ...
            function createHyperfiOutSchema(nearby, availableEmotes) {
                const lookAtSchema = nearby.length > 1
                    ? z
                        .union(nearby.map((item) => z.literal(item)))
                        .nullable()
                    : nearby.length === 1
                ...
                const emoteSchema = availableEmotes.length > 1
                    ? z
                        .union(availableEmotes.map((item) => z.literal(item)))
                        .nullable()
