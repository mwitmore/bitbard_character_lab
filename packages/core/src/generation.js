...
import { models, getModelSettings, getImageModelSettings, getEndpoint, } from "./models.ts";
import { parseBooleanFromText, parseJsonArrayFromText, parseJSONObjectFromText, parseShouldRespondFromText, parseActionResponseFromText, } from "./parsing.ts";
import settings from "./settings.ts";
import { ModelClass, ModelProviderName, ServiceType, TokenizerType, } from "./types.ts";
 ...
 */
export async function trimTokens(context, maxTokens, runtime) {
    if (!context)
        return "";
    if (maxTokens <= 0)
 ...
 */
export async function generateText({ runtime, context, modelClass, tools = {}, onStepFinish, maxSteps = 1, stop, customSystemPrompt, ragEnabled, }) {
    if (!context) {
    ...
    try {
        ...
        elizaLogger.debug(`Using provider: ${provider}, model: ${model}, temperature: ${temperature}, max response length: ${max_response_length}`);
        switch (provider) {
 ...
 */
export async function generateShouldRespond({ runtime, context, modelClass, }) {
    let retryDelay = 1000;
    while (true) {
        try {
 ...
 */
export async function splitChunks(content, chunkSize = 1500, bleed = 100) {
    elizaLogger.debug(`[splitChunks] Starting text split`);
    // Validate parameters
    if (chunkSize <= 0) {
        elizaLogger.warn(`Invalid chunkSize (${chunkSize}), using default 1500`);
        chunkSize = 1500;
    }
    if (bleed >= chunkSize) {
        elizaLogger.warn(`Bleed (${bleed}) >= chunkSize (${chunkSize}), adjusting bleed to 1/4 of chunkSize`);
...
export function splitText(content, chunkSize, bleed) {
    const chunks = [];
    let start = 0;
 ...
 */
export async function generateTrueOrFalse({ runtime, context = "", modelClass, }) {
    let retryDelay = 1000;
    const modelSettings = getModelSettings(runtime.modelProvider, modelClass);
 ...
 */
export async function generateTextArray({ runtime, context, modelClass, }) {
    if (!context) {
...
export async function generateObjectDeprecated({ runtime, context, modelClass, }) {
    if (!context) {
...
export async function generateObjectArray({ runtime, context, modelClass, }) {
    if (!context) {
 ...
 * @returns The completed message.
 */
export async function generateMessageResponse({ runtime, context, modelClass, ragEnabled, }) {
    const modelSettings = getModelSettings(runtime.modelProvider, modelClass);
    ...
    while (retryCount < MAX_RETRIES) {
        try {
            ...
            retryDelay = Math.min(INITIAL_RETRY_DELAY * Math.pow(2, retryCount) * (0.5 + Math.random()), 30000 // Max delay of 30 seconds
            );
            await new Promise((resolve) => setTimeout(resolve, retryDelay));
        }
    }
...
export const generateImage = async (data, runtime) => {
    ...
    try {
        ...
        else if (runtime.imageModelProvider === ModelProviderName.TOGETHER ||
...
export const generateCaption = async (data, runtime) => {
    const { imageUrl } = data;
    const imageDescriptionService = runtime.getService(ServiceType.IMAGE_DESCRIPTION);
 ...
 */
export const generateObject = async ({ runtime, context, modelClass, schema, schemaName, schemaDescription, stop, mode = "json", }) => {
    if (!context) {
 ...
 * @returns {Promise<any[]>} - A promise that resolves to an array of generated objects.
 */
export async function handleProvider(options) {
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleGrok({ model, apiKey, schema, schemaName, schemaDescription, mode = "json", modelOptions, runtime, }) {
    const grok = createOpenAI({
        apiKey,
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleGoogle({ model, apiKey, schema, schemaName, schemaDescription, mode = "json", modelOptions, runtime, }) {
    const google = createGoogleGenerativeAI({
        apiKey,
        fetch: runtime.fetch
    });
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleMistral({ model, schema, schemaName, schemaDescription, mode, modelOptions, runtime, }) {
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleRedPill({ model, apiKey, schema, schemaName, schemaDescription, mode = "json", modelOptions, runtime, }) {
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleOpenRouter({ model, apiKey, schema, schemaName, schemaDescription, mode = "json", modelOptions, runtime, }) {
    const openRouter = createOpenAI({
        apiKey,
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleOllama({ model, schema, schemaName, schemaDescription, mode = "json", modelOptions, provider, runtime, }) {
    const ollamaProvider = createOllama({
        baseURL: getEndpoint(provider) + "/api",
        fetch: runtime.fetch
    });
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleDeepSeek({ model, apiKey, schema, schemaName, schemaDescription, mode, modelOptions, runtime, }) {
    const openai = createOpenAI({
        apiKey,
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleBedrock({ model, schema, schemaName, schemaDescription, mode, modelOptions, provider, runtime, }) {
...
async function handleLivepeer({ model, apiKey, schema, schemaName, schemaDescription, mode, modelOptions, runtime, }) {
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleSecretAi({ model, apiKey, schema, schemaName, schemaDescription, mode = "json", modelOptions, provider, runtime, }) {
    const secretAiProvider = createOllama({
        baseURL: getEndpoint(provider) + "/api",
        headers: {
            "Content-Type": "application/json",
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleNearAi({ model, apiKey, schema, schemaName, schemaDescription, mode = "json", modelOptions, runtime, }) {
...
// doesn't belong here
export async function generateTweetActions({ runtime, context, modelClass, }) {
    let retryDelay = 1000;
    while (true) {
        try {
            const response = await generateText({
            ...
            elizaLogger.debug("Received response from generateText for tweet actions:", response);
