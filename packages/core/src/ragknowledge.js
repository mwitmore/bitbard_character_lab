            ...
            for (const item of parentDocuments) {
                ...
                if (!existsSync(filePath)) {
                    ...
                    try {
                        ...
                        elizaLogger.error(`[Cleanup] Error during deletion process for ${filePath}:`, deleteError instanceof Error
                            ? {
    ...
    async processFile(file) {
        ...
        try {
            ...
            for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
                ...
                const embeddings = await Promise.all(batch.map((chunk) => embed(this.runtime, chunk)));
                ...
                elizaLogger.info(`[Batch Progress] ${file.path}: Processed ${processedChunks}/${totalChunks} chunks (${batchTime.toFixed(2)}s for batch)`);
            }
