...
describe("Model Provider Configuration", () => {
    describe("OpenAI Provider", () => {
        ...
        test("should have correct model mappings", () => {
            ...
            expect(openAIModels[ModelClass.EMBEDDING].name).toBe("text-embedding-3-small");
    ...
    describe("Anthropic Provider", () => {
        ...
        test("should have correct model mappings", () => {
            ...
            expect(anthropicModels[ModelClass.SMALL].name).toBe("claude-3-haiku-20240307");
            expect(anthropicModels[ModelClass.MEDIUM].name).toBe("claude-3-5-sonnet-20241022");
            expect(anthropicModels[ModelClass.LARGE].name).toBe("claude-3-5-sonnet-20241022");
        });
        test("should have correct settings configuration", () => {
    ...
    describe("LlamaCloud Provider", () => {
        ...
        test("should have correct model mappings", () => {
            ...
            expect(llamaCloudModels[ModelClass.SMALL].name).toBe("meta-llama/Llama-3.2-3B-Instruct-Turbo");
            expect(llamaCloudModels[ModelClass.MEDIUM].name).toBe("meta-llama-3.1-8b-instruct");
            expect(llamaCloudModels[ModelClass.LARGE].name).toBe("meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo");
        });
        test("should have correct settings configuration", () => {
...
describe("Model Retrieval Functions", () => {
    describe("getModel function", () => {
        test("should retrieve correct models for different providers and classes", () => {
            expect(models[ModelProviderName.OPENAI].model[ModelClass.SMALL].name).toBe("gpt-4o-mini");
            expect(models[ModelProviderName.ANTHROPIC].model[ModelClass.MEDIUM].name).toBe("claude-3-5-sonnet-20241022");
        });
        test("should handle environment variable overrides", () => {
            expect(models[ModelProviderName.OPENROUTER].model[ModelClass.SMALL].name).toBe("nousresearch/hermes-3-llama-3.1-405b");
            expect(models[ModelProviderName.OPENROUTER].model[ModelClass.LARGE].name).toBe("nousresearch/hermes-3-llama-3.1-405b");
        });
        test("Test to ensure an invalid model provider returns undefined", () => {
            expect(getModelSettings("INVALID_PROVIDER", ModelClass.SMALL)).toBe(undefined);
        });
    });
    describe("getEndpoint function", () => {
        ...
        test("should throw error for invalid provider", () => {
            expect(() => getEndpoint("INVALID_PROVIDER")).toThrow();
        });
    });
});
...
describe("Environment Variable Integration", () => {
    test("should use environment variables for LlamaCloud models", () => {
        ...
        expect(llamaConfig.model[ModelClass.SMALL].name).toBe("meta-llama/Llama-3.2-3B-Instruct-Turbo");
    });
    test("should use environment variables for Together models", () => {
        const togetherConfig = models[ModelProviderName.TOGETHER];
        expect(togetherConfig.model[ModelClass.SMALL].name).toBe("meta-llama/Llama-3.2-3B-Instruct-Turbo");
    });
});
