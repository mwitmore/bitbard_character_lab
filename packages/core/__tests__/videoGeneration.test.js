    ...
    actions: [
        {
            ...
            handler: vi
                .fn()
                .mockImplementation(async (runtime, message, state, options, callback) => {
                // Initial response
                callback({
...
describe("Video Generation Plugin", () => {
    ...
    it("should validate when API key is present", async () => {
        const mockMessage = {};
        const result = await mockVideoGenerationPlugin.actions[0].validate(mockRuntime, mockMessage);
    ...
    it("should handle video generation request", async () => {
        ...
        const mockState = {};
        await mockVideoGenerationPlugin.actions[0].handler(mockRuntime, mockMessage, mockState, {}, mockCallback);
...
//# sourceMappingURL=videoGeneration.test.js.map